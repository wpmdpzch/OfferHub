...
class TestRunner:
    """Smoke test runner with proper state encapsulation."""
    ...
    def req(self, method: str, path: str, body: dict | None = None, token: str | None = None) -> tuple[dict, int]:
        """Make HTTP request to the API. Sequential execution, no concurrency issues."""
    ...
    def run(self) -> int:
        ...
        # 1. Health
        try:
            base_url = self.base.rsplit("/api/v1", 1)[0]
        ...
        # 2. 注册
        r, status = self.req("POST", "/auth/register", {
            "username": "smokeuser", "email": "smoke@example.com", "password": "SmokeTest123"
        })
        ...
        # 4. 登录
        r, status = self.req("POST", "/auth/login", {
        ...
        # 7. 文章列表
        r, status = self.req("GET", "/articles")
        self.check("GET /articles", r.get("code") == 0, str(r))

        # 8. 搜索
        q = urllib.parse.quote("字节")
        r, status = self.req("GET", f"/search?q={q}")
        self.check("GET /search", r.get("code") == 0, str(r))

        # 9. Refresh token
        r, status = self.req("POST", "/auth/refresh", {"refresh_token": refresh_token})
        self.check("POST /auth/refresh", r.get("code") == 0, str(r))

        # 10. 未认证访问需要认证的接口 -> 403
        r, status = self.req("POST", "/articles", {"title": "test", "content": "test"})
